...
export default function App() {
  ...
  return (
    <div className="fixed inset-0 flex bg-bg text-gray-900 dark:text-gray-100 overflow-hidden">
       ...
       {settingsOpen && <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={(e)=>{if(e.target===e.currentTarget) setSettingsOpen(false);}}><div className="w-full max-w-lg h-[80vh] bg-white dark:bg-gray-900 rounded-3xl overflow-hidden relative border shadow-2xl"><button onClick={()=>setSettingsOpen(false)} className="absolute top-4 right-4 p-2 bg-gray-200 dark:bg-gray-700 rounded-full z-10 hover:bg-gray-300 dark:hover:bg-gray-600"><X size={20}/></button><SettingsView currentUser={user} onLogout={()=>{setUser(null); localStorage.clear();}} isDark={document.documentElement.classList.contains('dark')} toggleTheme={()=>{document.documentElement.classList.toggle('dark'); localStorage.setItem('theme', document.documentElement.classList.contains('dark')?'dark':'light')}} onUpdate={setUser}/></div></div>}
    </div>
  );
}
